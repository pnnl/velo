...
public final class QName implements QNamePattern, Serializable, Cloneable, Comparable<QName>
{
    ...
    // CAT change:  not sure why Alfresco is hard coding the max name size to 255, but this conflicts with
    // file name property which can be any size.  So if you upload a file with a name > 255 characters, alfresco
    // will automatically truncate the assocation name without telling you, and then the name and association name
    // won't match, which causes all kinds of problems in CAT
    // For now, just increasing to 500.  CSL
