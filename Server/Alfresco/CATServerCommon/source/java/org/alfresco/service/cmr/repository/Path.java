...
public final class Path implements Iterable<Path.Element>, Serializable
{
    ...
    {
      // Velo change: call this other version that doesn't use permission service since it's expensive
      return toDisplayPath(nodeService);
...
//        return buf.toString();
    ...
    /**
     * Velo Change:  we want a version that doesn't take the permission service so it doesn't check permissions
     * for every node in the path, since that is expensive.
     * 
     * Another optimization is to assume each segment of the path (that uses assoc names) matches the node's name property.
     * we've hard coded exceptions to this rule and only for these known cases do we use nodeService to look up the node's name.
     * 
     * @return human readable form of the Path excluding the final element
     */
    private String toDisplayPath(NodeService nodeService)
    {
        StringBuilder buf = new StringBuilder(64);
        boolean companyHome = false;
        boolean categories = false;
        boolean useNameProp = true;
        ...
        for (int i=0; i<elements.size()-1; i++)
        {
            ...
            if (element instanceof ChildAssocElement)
            {
                ...
                //test the first segment of the path to see if it's company home or category root
                if(i == 1 &&elementRef.getQName().getLocalName().equals("company_home")) {
                  companyHome = true;
                }
                if(i == 1 &&elementRef.getQName().getLocalName().equals("categoryRoot")) {
                  categories = true;
                }
                
                //test the second segment of the path to see if its known to have a name property that does NOT match the assoc name 
                //and if so we have to look up the name property to return as part of the display path. 
                if(i == 2) {
                  if( companyHome && 
                      (elementRef.getQName().getLocalName().equals("dictionary")
                          || elementRef.getQName().getLocalName().equals("guest_home")
                          || elementRef.getQName().getLocalName().equals("sites") )
                      ) {
                    useNameProp = true;
                  } else if( categories && elementRef.getQName().getLocalName().equals("taggable") ) {
                    useNameProp = true;
                  } else {
                    useNameProp = false;
                  }
                ...
                if (elementRef.getParentRef() != null)
                {
                    ...
                    Serializable nameProp = null;
                    if(useNameProp)  // if a known exception to the rule that the assoc name matches the name property, we have to look up the name instead of using the assoc name
                    {
                        nameProp = nodeService.getProperty(elementRef.getChildRef(), ContentModel.PROP_NAME);
                        // use the name property if we are allowed access to it
                        elementString = nameProp.toString();
