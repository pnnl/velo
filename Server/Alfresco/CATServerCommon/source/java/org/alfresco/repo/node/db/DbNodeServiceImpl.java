...
public class DbNodeServiceImpl extends AbstractNodeServiceImpl
{
                ...
                boolean invokeOnUpdateProperties)
    {
        ...
        // Velo Change: extract the file processing pipeline information and queue, if applicable
        ContentData contentData = (ContentData)extraProperties.get(ContentModel.PROP_CONTENT);
        
        // include name changes in pipeline (MS Word creates a .tmp file, writes to it the new content, then renames the .tmp to the original filename.doc and so
        // for this case word docs weren't getting text transformed when changed via the velo file system drive mapping
        String newName = (String)extraProperties.get(ContentModel.PROP_NAME);
        String oldName = (String)existingProperties.get(ContentModel.PROP_NAME);
        ContentData oldContentData = (ContentData)existingProperties.get(ContentModel.PROP_CONTENT);
        
        //if old and new content data are both null, we don't need to include in pipeline as content didn't change
        if((newName != null && oldName != null && !oldName.equals(newName)) &&
           !(oldContentData == null && contentData == null)){
          ContentDataInfo info = DataSniffer.sniffContentData(newName, new byte[0]);
          
          if(contentData == null){
            contentData = new ContentData(oldContentData.getContentUrl(), info.getMimetype(), oldContentData.getSize(), oldContentData.getEncoding());
          }else{
            contentData = new ContentData(contentData.getContentUrl(), info.getMimetype(), contentData.getSize(), contentData.getEncoding());
          }
          extraProperties.put(ContentModel.PROP_CONTENT, contentData);
        }
        
        // only do this if we have content and it's not a thumbnail
        // TODO: try to filter out hidden renditions
        if(contentData != null) {          
          String fileName = (String)allProperties.get(ContentModel.PROP_NAME);
          
          // if fileName is null, this is probably because this is being called during a copy 
          // and since we are copying all the props, we don't need to rerun the file processing pipeline
          if(fileName != null) {
            FileProcessingInfo info = FileProcessingPipelineUtil.createFileProcessingInfo(nodeRef, fileName, contentData);
            if(info != null) {
              Map<QName, Serializable> transformProps = info.getProperties();

              for(QName key : transformProps.keySet()) {
                Serializable value = transformProps.get(key);
                allProperties.put(key, value);
                extraProperties.put(key, value);
              }

              // queue for pipeline
              FileProcessingPipelineUtil.addFileToPipeline(info);     
            } 
          } else {
            logger.warn("File name is null for node:  " + nodeRef.toString()); 
          }
        }     
        // end Velo change
