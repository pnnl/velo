...
public class LuceneCategoryServiceImpl implements CategoryService
{
    ...
    /**
     * CAT Change:  Alfresco was just throwing unsupported op exception, so we actually implement this method.
     * I did submit a bug report to alfresco about this issue, but they chose to ignore it.
     */
    public NodeRef createClassification(StoreRef storeRef, QName aspectName, String attributeName) {
      // Make sure the classification doesn't already exist
      Set<NodeRef> nodeRefs = getClassificationNodes(storeRef, aspectName);
      if (nodeRefs.size() > 0) {
        // TODO: if classification already exists, return existing one or throw exception?
      ...
      properties.put(ContentModel.PROP_NAME, aspectName.getLocalName());

      ChildAssociationRef childAssocRef = nodeService.createNode(parent, ContentModel.ASSOC_CATEGORIES, aspectName, ContentModel.TYPE_CATEGORY, properties);
    ...
    private ChildAssociationRef createCategoryInternal(NodeRef parent, String name)
    {
        ...
        String uri = nodeService.getPrimaryParent(parent).getQName().getNamespaceURI();
        String validLocalName = QName.createValidLocalName(name);
        // CAT Change:  allow any users to create a root category, so use unauthenticated node service
        ChildAssociationRef newCategory = nodeService.createNode(parent, ContentModel.ASSOC_SUBCATEGORIES, QName.createQName(uri, validLocalName), ContentModel.TYPE_CATEGORY);
        nodeService.setProperty(newCategory.getChildRef(), ContentModel.PROP_NAME, name);
        return newCategory;
    }
