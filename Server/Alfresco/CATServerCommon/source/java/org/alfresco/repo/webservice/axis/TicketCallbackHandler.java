   ...
   public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException
   {
      for (int i = 0; i < callbacks.length; i++) 
      {
         if (callbacks[i] instanceof WSPasswordCallback) 
         {
            ...
            try
            {
              // START CAT Change:  Alfresco only allows ticket validation when calling its axis web services,
              // but CAT needs username/password as well.  So we are modifying the ticket callback handler to
              // accept either tickets or username/password
              if(ticket != null && ticket.startsWith("TICKET_")) {
                ...
                this.authenticationService.authenticate(pc.getIdentifer(), ticket.toCharArray());
              }
              // END CAT Change
            }
