...
+    *            to find the node.  Otherwise, the path is used.
        ...
        if (uuid == null || uuid.length() == 0)
        {
            if (path != null && path.length() != 0)
            {
                ...
                if (nodes.size() != 1)
                {
                    ...
                    // CAT Change:  don't throw an exception if the node doesn't exist, instead, return  null
    ...
    {
      // CAT Change:  Make this work from outside axis context
        int batchSize = DEFAULT_BATCH_SIZE;
        MessageContext currentContext = MessageContext.getCurrentContext();
        Integer batchConfigSize = null;
        
        if(currentContext != null) {          
          batchConfigSize = (Integer) currentContext.getProperty(QueryConfigHandler.ALF_FETCH_SIZE);          
        }
