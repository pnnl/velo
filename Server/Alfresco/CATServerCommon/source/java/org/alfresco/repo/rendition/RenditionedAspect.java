                                          ...
                                          CopyServicePolicies.OnCopyNodePolicy
{
    ...
    {
      // CAT Change:  If a bootstrapped acp file has a node with a renditioned aspect, then this method crashes
      // because there is no secure authentication context during bootstrap
      // If we we are boostrapping, just ignore this method
      String username = AuthenticationUtil.getRunAsUser();

      // user could be unknown if doing the bootstrap
      if(username == null || username.equals("unknown")) {
        logger.warn("Skipping RenditionedAspect onUpdateProperties policy because of bootstrap");
        return;
      }
      // End cat change
