...
public class CopyServiceImpl implements CopyService
{
    ...
    /**
     * CAT Change:  added this as quick hack to support copying folders into a taxonomy, since we aren't supporting
     * linked folders yet
     * TODO: need to revisit taxonomy policy since it could be expensive
     * @param sourceNodeRef
     * @param destinationParent
     * @return
     */
    private NodeRef copyToTaxonomy(NodeRef sourceNodeRef, NodeRef destinationParent) {
      
      if(internalNodeService.getType(sourceNodeRef).equals(ContentModel.TYPE_FOLDER)) {
        String name = (String)internalNodeService.getProperty(sourceNodeRef, ContentModel.PROP_NAME);
        
        NodeRef newFolder = NodeUtils.createFolder(destinationParent, name, null, internalNodeService);
        for(ChildAssociationRef childRef : internalNodeService.getChildAssocs(sourceNodeRef)) {
          copyToTaxonomy(childRef.getChildRef(), newFolder);
        }
        return newFolder;
        
      } else if(internalNodeService.getType(sourceNodeRef).equals(ContentModel.TYPE_CONTENT)) {          
          // create link to source file
          return NodeUtils.createLinkedFile(sourceNodeRef, destinationParent, internalNodeService);  
      }
      return null;
    }
    ...
    public NodeRef copyAndRename(
            ...
            boolean copyChildren) 
    {
      // CAT Change: See if we are copying to a taxonomy:
      if(internalNodeService.hasAspect(destinationParent, CatConstants.ASPECT_TAXONOMY_ROOT) ||
          internalNodeService.hasAspect(destinationParent, CatConstants.ASPECT_TAXONOMY_FOLDER)) {
        return copyToTaxonomy(sourceNodeRef, destinationParent);        
      }
      // End CAT Change
        ...
        // Set name property
        // CAT Change: try to set the node with the destination name, instead of the source name, because
        // the caller may have renamed it
        this.internalNodeService.setProperty(result, ContentModel.PROP_NAME, assocQName.getLocalName());
                    ...
                    case COPY:
                        ...
                        switch (assocCopyAction.getSecond())
                        {
                            ...
                            case USE_COPIED_OTHERWISE_ORIGINAL_TARGET:
                              ...
                              // CAT Change: only add if it's not already in the list
                              // TODO: report this as an alfresco bug
                              // Copying nodes with associations was throwing a duplicate assoc exception
                              Pair<AssociationRef, AssocCopyTargetAction> pair = new Pair<AssociationRef, AssocCopyTargetAction>(assocRef, assocCopyAction.getSecond());
                              if(!postCopyAssocs.contains(pair)) {
                                System.out.println("adding new association to list: " + assocRef.toString());
                                postCopyAssocs.add(pair);
                              } else {
                                System.out.println("Found duplicate assoc in list so skipping: " + assocRef.toString());
                              }
                              // End cat change
                                break;
                            default:
