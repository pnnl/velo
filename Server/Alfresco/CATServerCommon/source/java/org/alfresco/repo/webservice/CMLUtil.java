...
public class CMLUtil
{
    ...
    private void executeCMLCreate(CMLCreate create, ExecutionContext context, List<UpdateResult> results)
    {
        ...
        // Create the new node
        // CAT change:  only if it doesn't already exist
        NodeRef nodeRef = nodeService.getChildByName(parentNodeRef, assocTypeQName, assocQName.getLocalName());
        if(nodeRef == null) {
          nodeRef = this.nodeService.createNode(parentNodeRef, assocTypeQName, assocQName, nodeTypeQName, properties).getChildRef();    
        }
    ...
    private void executeCMLDelete(CMLDelete delete, ExecutionContext context, List<UpdateResult> results)
    {
        ...
        for (NodeRef nodeRef : nodeRefs)
        {
          // CAT Change:  Create the result first so we have the path 
          UpdateResult result = createResult(DELETE, null, nodeRef, null);
          
          // CAT Change: make sure the node exists before we try to delete to prevent errors
    ...
    private void executeCMLMove(CMLMove move, ExecutionContext context, List<UpdateResult> results)
    {
        ...
        if (destinationNodeRef != null)
        {
            ...
            {
                // CAT Change:  Create the result first so we have the path 
                UpdateResult deletedNodeResult = createResult(DELETE, null, nodeToMove, null);

                NodeRef newNodeRef = this.nodeService.moveNode(nodeToMove, destinationNodeRef, assocType, assocName).getChildRef();
                
                // CAT Change:  Interpret results as a CREATE and a DELETE instead of a MOVE
                results.add(createResult(CREATE, null, null, newNodeRef));
                results.add(deletedNodeResult);
