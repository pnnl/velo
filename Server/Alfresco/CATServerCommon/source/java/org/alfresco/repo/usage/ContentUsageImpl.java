...
/**
 * CAT/OMICS CHANGE: let usage and quota data be stored not only on a person, but on a folder as well.
 * Then when quota is enabled **for spaces** instead of persons, crawl the node's primary parent tree
 * until the parent with the usage aspect is found, then increment/decement the space's usage property
 * and use the spaces quota property to figure out if its out of compliance  
 */
                                         ...
                                         NodeServicePolicies.OnCreateNodePolicy
{
    ...
    private boolean enabledForSpaces = true; //CAT CHANGE
    ...
    //CAT CHANGE BEGIN
    public void setEnabledForSpaces(boolean enabled)
    {
      this.enabledForSpaces = enabled;
    }
    //CAT CHANGE END
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef)
    {
        ...
        if (stores.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString()) && (! alreadyCreated(nodeRef)))
        {
            ...
            if (contentData != null)
            {
                ...
                //CAT CHANGE BEGIN
                if(this.enabledForSpaces){
                  //Get primary parent that has usage
//                  String owner = (String)nodeService.getProperty(nodeRef, ContentModel.PROP_OWNER);
                  NodeRef parentWithQuota = getParentWithQuota(nodeRef);
                  
                  if (contentSize != 0 && parentWithQuota != null && !nodeService.hasAspect(nodeRef, CatConstants.ASPECT_IGNORE))
                  {
                     // increment usage if node is being created
                     if (logger.isDebugEnabled()) logger.debug("onCreateNode: nodeRef="+nodeRef+", parentWithQuota="+parentWithQuota+", contentSize="+contentSize);
                     incrementSpaceUsage(parentWithQuota, contentSize, nodeRef);
    ...
    //CAT CHANGE BEGIN
    private NodeRef getParentWithQuota(NodeRef nodeRef) {
      return getParentWithQuotaRecursive(nodeRef);
    }
    
    private NodeRef getParentWithQuotaRecursive(NodeRef nodeRef) {
      ChildAssociationRef assoc = nodeService.getPrimaryParent(nodeRef);
      if(assoc.getParentRef() != null && nodeService.getProperty(assoc.getParentRef(), ContentModel.PROP_SIZE_CURRENT) != null){
        return assoc.getParentRef();
      }else if(assoc.getParentRef() != null ){
        return getParentWithQuotaRecursive(assoc.getParentRef());
      }

      return null;//no parent found with usage/quota props
    }
    ...
    //CAT CHANGE BEGIN
      //TODO  do we need to implement onMoveNode to move usage from old parent's prop to new parent's prop?  
      //      only applies if new parent has quota and if old parent isn't already getting usage decemented via other policy   
    //CAT CHANGE END
            ...
            Map<QName, Serializable> after)
    {
        if (stores.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString()) && (! alreadyCreated(nodeRef)))
        {
            ...
            //CAT CHANGE BEGIN
            if(this.enabledForSpaces && !nodeService.hasAspect(nodeRef, CatConstants.ASPECT_IGNORE)){
              NodeRef parentNodeRef = getParentWithQuota(nodeRef);//CAT uses parent node ref to get/set usage and quotas
              // check change in size
              if (contentSizeBefore == null && contentSizeAfter != null && contentSizeAfter != 0)
              {
                  // new size has been added
                  if (logger.isDebugEnabled()) logger.debug("onUpdateProperties: updateSize (null -> "+contentSizeAfter+"): nodeRef="+nodeRef);
                  incrementSpaceUsage(parentNodeRef, contentSizeAfter, nodeRef);
              }
              else if (contentSizeAfter == null && contentSizeBefore != null && contentSizeBefore != 0)
              {
                  // old size has been removed 
                  if (logger.isDebugEnabled()) logger.debug("onUpdateProperties: updateSize ("+contentSizeBefore+" -> null): nodeRef="+nodeRef);
                  decrementSpaceUsage(parentNodeRef, contentSizeBefore, nodeRef);
              }
              else if (contentSizeBefore != null && contentSizeAfter != null)
              {
                  ...
                  {
                      // size has changed
                      if (logger.isDebugEnabled()) logger.debug("onUpdateProperties: updateSize ("+contentSizeBefore+" -> "+contentSizeAfter+"): nodeRef="+nodeRef);
    ...
    public void beforeDeleteNode(NodeRef nodeRef)
    {
        if (stores.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString()) && (! alreadyDeleted(nodeRef)))
        {
            ...
            if (contentData != null)
            {
                ...
                //CAT CHANGE BEGIN
                if(this.enabledForSpaces && !nodeService.hasAspect(nodeRef, CatConstants.ASPECT_IGNORE)){
                  // Get parent that stores the usage/quota
                  NodeRef parentNodeRef = getParentWithQuota(nodeRef);
                  if (contentSize != 0 && parentNodeRef != null)
                  {
                     // decrement usage if node is being deleted
                     if (logger.isDebugEnabled()) logger.debug("beforeDeleteNode: nodeRef="+nodeRef+", parentNodeRef="+parentNodeRef+", contentSize="+contentSize);
                     decrementSpaceUsage(parentNodeRef, contentSize, nodeRef);
    ...
    */
    ...
    //CAT CHANGE BEGIN
    private void incrementSpaceUsage(NodeRef nodeRef, long contentSize, NodeRef contentNodeRef)
    {
            // increment usage - add positive delta
            if (logger.isDebugEnabled()) logger.debug("incrementSpaceUsage: nodeRef="+nodeRef+", contentSize="+contentSize+", contentNodeRef="+contentNodeRef);
            
            long currentSize = getSpaceUsage(nodeRef);
            long quotaSize = getSpaceQuota(nodeRef);
            ...
            if ((quotaSize != -1) && (newSize > quotaSize))
            {
                if (logger.isWarnEnabled())
                {
                    logger.warn("NodeRef (" + nodeRef + ") quota exceeded: content=" + contentSize +
                                  ", usage=" + currentSize +
                                  ", quota=" + quotaSize);
                }
                throw new ContentQuotaException("Space quota exceeded");
            }
            
            if (nodeRef != null)
            {
                usageService.insertDelta(nodeRef, contentSize);
            }
    }
    //CAT CHANGE END
    ...
    //CAT CHANGE BEGIN
    private void decrementSpaceUsage(NodeRef nodeRef, long contentSize, NodeRef contentNodeRef)
    {
        // decrement usage - add negative delta
        if (logger.isDebugEnabled()) logger.debug("decrementSpaceUsage: nodeRef="+nodeRef+", contentSize="+contentSize+", contentNodeRef="+contentNodeRef);
        ...
        if (newSize < 0)
        {
          if (logger.isDebugEnabled())
          {
            logger.debug("nodeRef (" + nodeRef + ") has negative usage (" + newSize + ") - reset to 0");
          }
        }
    ...
    //CAT NOTE this works for nodeRefs that are not person's as well, using same method for when this.enabledForSpaces is true
    ...
    //CAT CHANGE BEGIN
    public long getSpaceUsage(NodeRef nodeRef)
    {
      return getUserUsage(nodeRef, false);
    }
    //CAT CHANGE END
        ...
        if (currentUsage != -1)
        {
          ...
          currentUsage = currentUsage + deltaSize;
            ...
            if (currentUsage < 0)
            {
                if (logger.isWarnEnabled())
                {
                    //CAT CHANGE to add /Space to logging
                    logger.warn("User/Spaces usage ("+ personNodeRef+") is negative ("+currentUsage+") overriding to 0");
                }
                currentUsage = 0;
            }
        }
    ...
    //CAT NOTE: not supported for this.enabledForSpaces
    /**
    ...
    //CAT CHANGE BEGING
    public long getSpaceQuota(NodeRef nodeRef)
    {
      Long currentQuota = null;
      
      if (nodeRef != null)
      {
        currentQuota = (Long)nodeService.getProperty(nodeRef, ContentModel.PROP_SIZE_QUOTA);
      }
