...
//CAT is overwriting this class in order to have metadata extraction run in a seperate process, 
//thus greatly improving performance.  We had to overwrite this class for 2 reasons:
//1. since we serialize the metadata extractor object to a file for the seperate process to deserialize
// and then use, we made services transient (we noticed they were not used in the extract process)
//2. the mimetypeService was indeed used while extracting, but only to make an error message when an
// extraction failed.  So we commented out that message bit

abstract public class AbstractMappingMetadataExtracter implements MetadataExtracter, MetadataEmbedder, BeanNameAware, Serializable
{//CAT CHANGE: added Serializable to interfaces it implements
    ...
    transient protected static Log logger = LogFactory.getLog(AbstractMappingMetadataExtracter.class);
    
    transient private MetadataExtracterRegistry registry;
    ...
    private Map<QName, Serializable> mapRawToSystem(Map<String, Serializable> rawMetadata)
    {
        ...
        for (Map.Entry<String, Serializable> entry : rawMetadata.entrySet())
        {
            ...
            {
                // No mapping - ignore
              System.out.println("no mapping found for " + documentKey);
              System.err.println("no mapping found for " + documentKey);              
                continue;
            }
...
//        }
    ...
    //CAT CHANGE change this method to public so we can call it from the MetadataExtractionProcessor
    public Map<QName, Serializable> convertSystemPropertyValues(Map<QName, Serializable> systemProperties)
    {
...
//                        {
                            ...
                            System.err.println(mesg.toString());
//                        }
                    }
                }
                else
            ...
            catch (MalformedNodeRefException e)
            {
                ...
                else
                {
                    if (failOnTypeConversion)
                    {
                      System.out.println("failOnTypeConversion true, thowing ERR_TYPE_CONVERSION exception");
                        throw e;
                    }
                }
            }
        ...
        // Done
        QName dateQName = QName.createQName("{http://www.alfresco.org/model/content/1.0}sentdate");
        Object obj = convertedProperties.get(dateQName);
        if(obj != null){
