 ...
 */
public abstract class AbstractVeloWebScript extends AbstractWebScript {
  protected static Log logger = LogFactory.getLog(AbstractVeloWebScript.class);
  ...
  public void execute(final WebScriptRequest req, final WebScriptResponse res) throws IOException {
    try {
      ...
      final File requestContent = TempFileProvider.createTempFile("velo_webscript_input_", ".bin");
   ...
   * @param e Throwable
   */
  protected void handleError(WebScriptResponse res, Throwable e) {
    // Get the root cause
    Throwable rootCause = getRootCause(e);
    
    if(rootCause instanceof DuplicateChildNodeNameException ||
        rootCause instanceof DuplicateKeyException) {
      res.setStatus(VeloServletReponseCodes.SC_DUPLICATE_NODE);
      
    } else if (rootCause.toString().contains("Duplicate")) {
      res.setStatus(VeloServletReponseCodes.SC_DUPLICATE_NODE);
      
    } else if(rootCause instanceof ProvenanceException){
      res.setStatus(VeloServletReponseCodes.SC_NONDELETABLE_RELATIONSHIPS);
      writeErrorCause(res, e);
    } else {
  ...
  /**
   * Method writeErrorCause.
   * @param res WebScriptResponse
   * @param e Throwable
   */
  protected void writeErrorCause(WebScriptResponse res, Throwable e) {
