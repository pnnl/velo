...
import gov.pnnl.cat.server.webservice.user.UserFilter;
...
/**
 * Query users.  For now this web script just forwards calls to UserWebService to
 * avoid axis calls from client.
 * @author d3k339
 *
 * @version $Revision: 1.0 $
 */
public class QueryUsers extends AbstractCatWebScript {
  ...
  protected Object executeImpl(WebScriptRequest req, WebScriptResponse res, File requestContent) throws Exception {
    ...
    if(requestContent.length() > 0) {
      filter = mapper.readValue(requestContent, UserFilter.class);
    }
    UserQueryResults queryResults = userWebService.queryUsersImpl(filter);
