...
public class JarLibrary {
  ...
  public void addJars(final String filePath) {
    addJars(filePath, false);
  }

  public void addJars(final String filePath, final boolean recurse) {
    addJars(new File(filePath), recurse);
  }
  ...
  public void addJars(final File file, final boolean recurse) {
    if (file.isDirectory()) {
      for (File fileEntry : file.listFiles()) {
        if (recurse) {
          addJars(fileEntry, recurse);
        } else {
          addJar(fileEntry);
        }
      }
