...
public class CodeExecutor {
   ...
   * @throws ExecException  - when the job execution fails with exceptions (eg. IOException, SecurityException)
   */
  public static int execute(final String codeID, Object commandArgs, final File localWorkingDir,
      boolean backgroundProcess, final boolean useWrapperShell) throws ExecException  {
    int exitCode = 0;
    
    if(commandArgs instanceof String[]) {
      //String cmdArgsStr =  "--infile="+  "\""+  new File(tempDirectory, DESIGN_INPUT_FILE_NAME).getAbsolutePath() + "\"";  //works on windows NOT on mac
      //String[] cmdArgsStr =  {"--infile="+  "\""+  new File(tempDirectory, DESIGN_INPUT_FILE_NAME).getAbsolutePath() + "\""}; //does NOT work on windows, works on mac
      //so to fix above situation, for windows we have to use double quotes in args, but only if the args are passed in as an array. 
      //re-consider using Alfresco's org.alfresco.util.exec.ExecParameterTokenizer class if this logic needs additional work in the future
      String[] commandArgsArray = (String[])commandArgs;
      for (int i = 0; i < commandArgsArray.length; i++) {
        //for windows OS's replace any single quotes with double quotes
        ((String[])commandArgs)[i] = quoteArg(((String[])commandArgs)[i]);  
      }
    }
    
    final Object commandArgsFinal = commandArgs;
    ...
    if(!backgroundProcess) {
      ...
      Thread thread = new Thread(new Runnable() {
        ...
        @Override
        public void run() {
          executeInternal(codeID, commandArgsFinal, localWorkingDir, useWrapperShell);        
        }
      } );
      thread.start();
    }
  ...
  private static String quoteArg(String arg) {
    ...
    if(osName.contains("windows") && arg.contains("\"")){
      arg = arg.replaceAll("\"", "\"\"");
    }
    return arg;
  }
...
//    }
  ...
  public static String getPathToExecutable(String codeID) {
    ...
    if(command.startsWith("${exeDirPath}")) {
      ...
      // then look in the global properties to see if the command has been set by the installer
      String key = codeID + ".command";
      command = ResourcesPlugin.getVeloGlobalProperties().getProperties().getProperty(key);
    }
