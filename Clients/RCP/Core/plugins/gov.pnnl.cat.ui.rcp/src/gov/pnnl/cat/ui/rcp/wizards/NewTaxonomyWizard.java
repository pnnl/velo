...
public class NewTaxonomyWizard extends Wizard implements INewWizard {
   ...
   */
  public NewTaxonomyWizard() {
    super();
    setNeedsProgressMonitor(true);
    setWindowTitle("New "+CatRcpMessages.NewTaxonomy_window_title+"...");
    setDefaultPageImageDescriptor(SharedImages.getInstance().getImageDescriptor(SharedImages.CAT_IMG_TAXONOMY_NEW, SharedImages.CAT_IMG_SIZE_64));
  }
  ...
  public void addPages() {
    page = new NewFolderWizardPage(selection, IResource.TAXONOMY_ROOT, this.workbenchWindow);
    addPage(page);
  }
  ...
  public boolean performFinish() {
    success = false;
//    final String taxonomyName = page.getTaxonomyName();
//    final String taxonomyDisplayName = page.getTaxonomyDisplayName();
//    final String taxonomyDescription = page.getTaxonomyDescription();
//    
  ...
  private boolean doFinish(CmsPath parentFolder, String newFolderName,  IProgressMonitor monitor) {
    ...
    try {
      ...
      if (mgr.resourceExists(newFolderPath)) {
        String errMsg = CatRcpMessages.NewTaxonomy_already_exists_title + " " + CatRcpMessages.NewTaxonomy_already_exists_message;
        ToolErrorHandler.handleError(errMsg, null, true);
        return false;
      }
      mgr.createTaxonomy(newFolderPath);
      monitor.worked(1);
      
    } catch (Throwable e) {
      String errMsg = CatRcpMessages.NewTaxonomy_cannot_create_message;
      ToolErrorHandler.handleError(errMsg, e, true);
      return false;
    }
    return true;
