...
public class ScopePart {
	...
	public ScopePart(SearchDialog searchDialog, boolean searchEnclosingProjects) {
		IDialogSettings dialogSettings= CatRcpPlugin.getDefault().getDialogSettings();
	...
	/**
	 * Method getStoredScope.
	 * @param settingsStore IDialogSettings
	 * @return int
	 */
	...
	public void setSelectedScope(int scope) {
		...
		if (scope == ISearchPageContainer.SELECTED_PROJECTS_SCOPE) {
			if (!fCanSearchEnclosingProjects) {
				CatSearchPlugin.log(new Status(IStatus.WARNING, NewSearchUI.PLUGIN_ID, IStatus.WARNING, "Enclosing projects scope set on search page that does not support it", null)); //$NON-NLS-1$
				scope= ISearchPageContainer.WORKSPACE_SCOPE;
	...
	private boolean hasFocusEditor() {
		IWorkbenchPage activePage= CatSearchPlugin.getActivePage();
		if (activePage == null)
			return false;
