...
import gov.pnnl.cat.search.eclipse.search.internal.ui.SelectAllAction;
...
import gov.pnnl.cat.search.eclipse.search2.internal.ui.SearchResultsView;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.CollapseAllAction;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.ExpandAllAction;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.INavigate;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.RemoveAllMatchesAction;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.SetLayoutAction;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.TableViewerNavigator;
import gov.pnnl.cat.search.eclipse.search2.internal.ui.basic.views.TreeViewerNavigator;
...
public abstract class AbstractTextSearchViewPage extends Page implements ISearchResultPage {
	...
	/**
	 */
	private class SelectionProviderAdapter implements IPostSelectionProvider {
		private ArrayList fListeners= new ArrayList(5);
		private ArrayList fPostListeners= new ArrayList(5);
      ...
      public void selectionChanged(SelectionChangedEvent event) {
        ...
        SelectionChangedEvent wrappedEvent= new SelectionChangedEvent(SelectionProviderAdapter.this, event.getSelection());
		...
		private ISelectionChangedListener fPostSelectionChangedListener = new ISelectionChangedListener() {
		  public void selectionChanged(SelectionChangedEvent event) {
		    // forward to my listeners
		    SelectionChangedEvent wrappedEvent= new SelectionChangedEvent(SelectionProviderAdapter.this, event.getSelection());
		    for (Iterator listeners= fPostListeners.iterator(); listeners.hasNext();) {
		      ISelectionChangedListener listener= (ISelectionChangedListener) listeners.next();
		      listener.selectionChanged(wrappedEvent);
		    }
		  }
    ...
    /**
     * Method getSelectionChangedListener.
    ...
    /**
     * Method getPostSelectionChangedListener.
     * @return ISelectionChangedListener
     */
    public ISelectionChangedListener getPostSelectionChangedListener() {
      return fPostSelectionChangedListener;
    }
...
//	private Action fShowPreviousAction;
	 ...
	 * @see #FLAG_LAYOUT_FLAT * @see #FLAG_LAYOUT_TREE */
...
//		if (countBits(fSupportedLayouts) > 1) {
      ...
      fHierarchicalAction.setEnabled(isLayoutSupported(FLAG_LAYOUT_TREE));
      fFlatAction.setEnabled(isLayoutSupported(FLAG_LAYOUT_FLAT));
//		}
	}
	
	/**
	 * Method countBits.
	 * @param layoutFlags int
	 * @return int
	 */
	 ...
	 * @return the dialog settings for this search result page * @see AbstractTextSearchViewPage#getID() */
	protected IDialogSettings getSettings() {
		IDialogSettings parent = CatRcpPlugin.getDefault().getDialogSettings();
	 ...
	 *             if an editor can't be opened * @see org.eclipse.core.filebuffers.ITextFileBufferManager * @see IFileMatchAdapter */
...
//			mgr.appendToGroup(IContextMenuConstants.GROUP_REMOVE_MATCHES, fRemoveSelectedMatches);
	...
	/**
	 * Method showBusyLabel.
	 * @param shouldShowBusy boolean
	 */
	 ...
	 * @return whether the given layout is supported or not * @see AbstractTextSearchViewPage#AbstractTextSearchViewPage(int) */
	...
	private void disposeViewer() {
		fViewer.removeSelectionChangedListener(fViewerAdapter.getSelectionChangedListener());
		fViewer.removePostSelectionChangedListener(fViewerAdapter.getPostSelectionChangedListener());
	 ...
	 * @return the layout this page is currently using * @see #FLAG_LAYOUT_FLAT * @see #FLAG_LAYOUT_TREE */
...
//		fCopyToClipboardAction.setViewer(fViewer);
		...
		tbm.removeAll();
		SearchResultsView.createStandardGroups(tbm, getSite().getWorkbenchWindow());
		fillToolbar(tbm);
		tbm.update(false);
...
//				fRemoveSelectedMatches.setEnabled(!event.getSelection().isEmpty());
		...
		fViewer.addSelectionChangedListener(fViewerAdapter.getSelectionChangedListener());
		fViewer.addPostSelectionChangedListener(fViewerAdapter.getPostSelectionChangedListener());
	 ...
	 *         created yet. * @see gov.pnnl.cat.search.eclipse.search.ui.ISearchResultPage#getViewer()
	 */
	 ...
	 * @return the previously set result or <code>null</code> * @see AbstractTextSearchViewPage#setInput(ISearchResult, Object) */
	 ...
	 *         of this page is <code>null</code>, an empty array is returned * @see AbstractTextSearchResult#getMatches(Object) */
	 ...
	 *         returned * @see AbstractTextSearchResult#getMatchCount(Object) */
	...
	/**
	 * Method getFirstSelectedElement.
	 * @return Object
	 */
	...
	/**
	 * Method initActionDefinitionIDs.
...
//			actionBars.setGlobalActionHandler(ActionFactory.COPY.getId(), fCopyToClipboardAction);
      ...
      new ActionDelegateViewSetup(actionBars);
		}
  ...
  protected synchronized void handleSearchResultsChanged(final SearchResultEvent e) {
	...
	/**
	 * Method collectAllMatchesBelow.
	 * @param result AbstractTextSearchResult
	 * @param set Set
	 * @param cp ITreeContentProvider
	 * @param elements Object[]
	 */
