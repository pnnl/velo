package org.apache.commons.httpclient.old;
...
public class URI implements Cloneable, Comparable, Serializable {
     ...
     * 
     * @throws Exception If the URI cannot be created.
     ...
     * @deprecated Use #URI(String, boolean, String)
     */
    public URI(String original, String charset) throws Exception {
        protocolCharset = charset;
     ...
     * @param fragment the fragment string
     * @throws Exception If the new URI cannot be created.
     * @see #getDefaultProtocolCharset
     */
    public URI(String scheme, String authority, String path, String query,
        ...
        if (path != null) {  // accept empty path
            if ((scheme != null || authority != null)
                    && !path.startsWith("/")) {
                throw new Exception("abs_path requested");
            }
            buff.append(path);
        }
        if (query != null) {
     ...
     */
    public URI(URI base, String relative) throws Exception {
     ...
     * @since 3.0
     */
    public URI(URI base, String relative, boolean escaped) throws Exception {
    ...
    public URI(URI base, URI relative) throws Exception {
        ...
        if (base._scheme == null) {
            throw new Exception("base URI required");
        }
        if (base._scheme != null) {
     ...
     * @throws Exception null component or unsupported character encoding
     */
     ...
     * @throws Exception incomplete trailing escape pattern or unsupported
     * character encoding
     */
    ...
    protected void parseUriReference(String original, boolean escaped)
        ...
        // validate and contruct the URI character sequence
        if (original == null) {
            throw new Exception("URI-Reference required");
        }
    ...
    protected void parseAuthority(String original, boolean escaped)
        ...
        if (next >= from) {
            next = original.indexOf(']', from);
            if (next == -1) {
                throw new Exception("IPv6reference");
            } else {
                next++;
            }
     ...
     * @throws Exception If {@link #decode} fails
     */
     ...
     * @throws Exception encoding error or not proper for initial instance
     * @see #encode
     */
    public void setRawPath(char[] escapedPath) throws Exception {
        ...
        if (_is_net_path || _is_abs_path) {
            ...
            if (!validate(escapedPath, abs_path)) {
                throw new Exception("escaped absolute path not valid");
            }
            _path = escapedPath;
            ...
            if (at > 0 && !validate(escapedPath, 0, at - 1, rel_segment) 
                ...
                throw new Exception("escaped relative path not valid");
            }
            _path = escapedPath;
        } else if (_is_opaque_part) {
            if (!uric_no_slash.get(escapedPath[0]) 
                && !validate(escapedPath, 1, -1, uric)) {
                throw new Exception("escaped opaque part not valid");
            }
            _opaque = escapedPath;
        } else {
            throw new Exception("incorrect path");
     ...
     * @see #encode
     */
    public void setEscapedPath(String escapedPath) throws Exception {
        if (escapedPath == null) {
            _path = _opaque = null;
            setURI();
            return;
     ...
     *
     * @param path the path string
     * @throws Exception set incorrectly or fragment only
     * @see #encode
     */
    public void setPath(String path) throws Exception {
        ...
        if (_is_net_path || _is_abs_path) {
            ...
            int at = path.indexOf('/');
            if (at == 0) { // never 0
                throw new Exception("incorrect relative path");
            }
            if (at > 0) {
     ...
     * @throws Exception no more higher path level to be resolved
     */
     ...
     * @throws Exception no hierarchy level
     */
    protected char[] getRawCurrentHierPath(char[] path) throws Exception {

        if (_is_opaque_part) {
            throw new Exception("no hierarchy level");
        }
        if (path == null) {
     ...
     * @throws Exception If {@link #getRawCurrentHierPath(char[])} fails.
     */
     ...
     * @throws Exception escaped query not valid
     */
    public void setRawQuery(char[] escapedQuery) throws Exception {
        ...
        if (!validate(escapedQuery, query)) {
            throw new Exception("escaped query not valid");
        }
        _query = escapedQuery;
        setURI();
    }
     ...
     * @throws Exception escaped query not valid
     */
    public void setEscapedQuery(String escapedQuery) throws Exception {
     ...
     * @throws Exception escaped fragment not valid
     */
    public void setRawFragment(char[] escapedFragment) throws Exception {
        ...
        if (!validate(escapedFragment, fragment)) {
            throw new Exception("escaped fragment not valid");
        }
        _fragment = escapedFragment;
        hash = 0;
    }
     ...
     * @throws Exception escaped fragment not valid
     */
    public void setEscapedFragment(String escapedFragment) throws Exception {
     ...
     * @throws Exception no more higher path level to be normalized
     */
    protected char[] normalize(char[] path) throws Exception {
     ...
     * @throws Exception If {@link #decode} fails.
     */
    public String getURIReference() throws Exception {
        char[] uriReference = getRawURIReference();
