...
public final class AwtEnvironment {
  ...
  // CAT Change
  // For some deployments, if we are not using embedded swing, but we are using swing windows
  // and swt windows, if we try to sync modal, it causes the RCP window to pop in front of the
  // swing window, which is undesirable
  private static boolean syncModalDialogs = true;
     ...
     * @return the syncModalDialogs */
    public static boolean isSyncModalDialogs() {
      return syncModalDialogs;
    }

    /**
     * @param syncModalDialogs the syncModalDialogs to set
     */
    public static void setSyncModalDialogs(boolean syncModalDialogs) {
      AwtEnvironment.syncModalDialogs = syncModalDialogs;
    }
    
    /**
     * Velo change: we are adding this method so components can check if the environment
     * has been initialized before rendering.  On macs, calling getInstance() too early
     * in the startup process (i.e., when restoring views) can lock up the UI.  Checking
     * if the instance has been instantiated yet and only rendering if true can fix this
     * problem.
     * @param display
     * @return
     */
    public static boolean isInitialized(final Display display) {
     ...
     * @return the AWT environment * @exception IllegalArgumentException
     *                <ul>
    ...
    private GlobalFocusHandler globalFocusHandler;
    

    // Private constructor - clients use getInstance() to obtain instances
    /**
     * Constructor for AwtEnvironment.
     * @param display Display
     */
    private AwtEnvironment(final Display display) {
        ...
        try {
            // Velo change:  on newer macs, this deadlocks, so
            // we can't do EventQueue.invokeAndWait
        ...
        // Velo change:  to avoid deadlocks on macs, we run this asynchronously too
     ...
     *                SWT event thread
     *                </ul> * @exception IllegalArgumentException
     *                <ul>
     ...
     * @return a {@link java.awt.Frame} to be used for parenting dialogs * @exception IllegalStateException
     *                if the current display has no shells * @exception SWTException
     *                <ul>
     *                <li>ERROR_THREAD_INVALID_ACCESS - if not called from the
     *                SWT event thread
     *                </ul> */
