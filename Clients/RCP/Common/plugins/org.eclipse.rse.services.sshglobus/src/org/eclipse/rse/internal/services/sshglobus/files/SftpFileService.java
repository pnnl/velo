...
package org.eclipse.rse.internal.services.sshglobus.files;
...
import org.eclipse.rse.internal.services.sshglobus.Activator;
import org.eclipse.rse.internal.services.sshglobus.ISshService;
import org.eclipse.rse.internal.services.sshglobus.ISshSessionProvider;
import org.eclipse.rse.internal.services.sshglobus.SshServiceResources;
...
public class SftpFileService extends AbstractFileService implements ISshService, IFilePermissionsService
{
  ...
  {
    // for globus online transfers the localFile will not exists since it has not been downloaded
  ...
  // ========================================================================
  /*
	public static class GlobusCredentialsProvider {

		private GlobusOnlineTransferer transferer = new GlobusOnlineTransferer();
		private IHost globusHost;
		private SshConnectorService connectorService;
		private static GlobusCredentialsProvider INSTANCE;
		private ICredentials credentials;

		private String aliasName = "Globus Online Server";
		//private String urlString = "https://nexus.api.globusonline.org/goauth/token?grant_type=client_credentials";
		private String hostName = "nexus.api.globusonline.org";

		public static GlobusCredentialsProvider getInstance(StandardConnectorService gocs) {
			if(INSTANCE == null)
				INSTANCE = new GlobusCredentialsProvider(gocs);
			return INSTANCE;
		}

		private GlobusCredentialsProvider(StandardConnectorService gocs) {
			connectorService = gocs;
		}

		public ICredentials acquireGlobusOnlineCredentials(boolean reacquire) {
			if(credentials == null) {

				// create the host for the globus connection
				IHost host = connectorService.getHost();
				globusHost = host.getHostPool().getHost(aliasName);
				if(globusHost == null) {
					globusHost = host.getHostPool().createHost(host.getSystemType(), aliasName, hostName);
				}

				// create the credentialsProvider and request the username and password
				ICredentialsProvider globusCredentialsProvider = new StandardCredentialsProvider(connectorService);
				connectorService.setDenyPasswordSave(true);
				connectorService.setHost(globusHost);

				globusCredentialsProvider.acquireCredentials(reacquire);  // this line prompts for the password
				credentials = globusCredentialsProvider.getCredentials();

				// reset the original host (non globus host)
				connectorService.setHost(host);
				host.getHostPool().deleteHost(globusHost); // SEE IF THIS WORKS?
			}
			return credentials;
		}

		public boolean connectToGlobusOnline() {
			try {
				transferer.authenticateToGlobusOnline(credentials.getUserId(), credentials.getPassword());
			} catch (Exception e) {
				credentials = null;
				return false;
			}
		...
		public JSONTransferAPIClient connectToGlobusEndpoint() throws OperationCanceledException {
			boolean success = false;
			JSONTransferAPIClient client =null; 

			do {
				IPropertySet propertySet = connectorService.getPropertySet();
				String portStr = propertySet.getProperty("port").getValue();
				SystemSignonInformation info = connectorService.getSshSignonInformation();
				String hostname = info.getHostname().toLowerCase();
				if(portStr != null) hostname += ":" + portStr;

				try {
					client = transferer.autoActivateEndpoint(
							propertySet.getProperty("globusEndpoint").getValue().toLowerCase());
					success = client != null;
				...
				if(! success) {
					try {
						System.out.println("connectToGlobusEndpoint :: acquireCredentials");
						connectorService.setSuppressed(false);
						connectorService.acquireCredentials(true);	
						info = connectorService.getSignonInformation();

						System.out.println("connectToGlobusEndpoint :: activateEndpoint");
						client = transferer.activateEndpoint(
							propertySet.getProperty("globusEndpoint").getValue().toLowerCase(), 
							hostname, info.getUserId(), info.getPassword());
						success = client != null;
						System.out.println("connectToGlobusEndpoint :: activateEndpoint success = " + success);
	...
	protected ICredentials globusOnlineCreds;

	public void acquireCredentialsForGlobusOnline(boolean reacquire) throws OperationCanceledException {
//		ICredentials credentials = null;
		IConnectorService host = getConnectorService("nexus.api.globusonline.org");

		do {
			globusOnlineCreds = GlobusCredentialsProvider.getInstance(host).acquireGlobusOnlineCredentials(reacquire);
		} while(! GlobusCredentialsProvider.getInstance(host).connectToGlobusOnline());

//		((SftpFileService)getFileService()).setGlobusUserId(credentials.getUserId());
//		((GlobusOnlineService)getFileService()).setGlobusPassword(credentials.getPassword());
	}
   */
  ...
  public static IConnectorService getConnectorService(String hostname) {
    ...
    if(ss instanceof IFileServiceSubSystem) {
      ...
      IConnectorService connSvc = fss.getConnectorService();
      return connSvc;
    }
    return null;
  }

  public JSONTransferAPIClient connectToGlobusEndpoint(String endpoint, String hostname, String username, String password) {
    boolean success = false;
    JSONTransferAPIClient client =null; 
    ...
    try {
      client = transferer.autoActivateEndpoint(endpoint);
      success = client != null;
      System.out.println("connectToGlobusEndpoint :: autoActivateEndpoint success = " + success);
    ...
    if(! success) {
      try {
        ...
        System.out.println("connectToGlobusEndpoint :: activateEndpoint");
        client = transferer.activateEndpoint(endpoint, 
  ...
  public static String getGlobusOnlineHostProperty(IHost host, String propName) {
    IRemoteFileSubSystem ss = RemoteFileUtility.getFileSubSystem(host);
    if(ss instanceof IFileServiceSubSystem) {
      IFileServiceSubSystem fss = (IFileServiceSubSystem)ss;
      IConnectorService connSvc = fss.getConnectorService();
      IPropertySet propSet = connSvc.getPropertySet(IGlobusOnlineConnectorService.PROPERTY_SET_GLOBUS);
      if(propSet != null) {
        IProperty sourceGlobusEndpointProp = propSet.getProperty(propName);
        String propVal = sourceGlobusEndpointProp.getValue();
        return propVal;
      }
    } else {
  ...
  public static String getEndpoint(String hostname) {
    if("hopper.nersc.gov".equals(hostname)) return "nersc#hopper";
    if("olympus.pnl.gov".equals(hostname)) return "pic#dtn";
    if("dolson#scdemo_data".equals(hostname)) return "dolson#scdemo_data";
    if("ranantha#cfngftp".equals(hostname)) return "ranantha#cfngftp";
    return null;
  }

  public static String getMyProxyHost(String endpoint) {
    if("pic#dtn".equals(endpoint)) return "dtn.pnl.gov:7512";
    if("nersc#hopper".equals(endpoint)) return "nerscca.nersc.gov";
    if("dolson#scdemo_data".equals(endpoint)) return "140.221.66.2";
    if("ranantha#cfngftp".equals(endpoint)) return "nerscca1.nersc.gov";
    return null;
  }

  public static String getUsername(String endpoint) {
    if("pic#dtn".equals(endpoint)) return "d3k339";
    if("nersc#hopper".equals(endpoint)) return "carinal";
    if("dolson#scdemo_data".equals(endpoint)) return "clansing";
    if("ranantha#cfngftp".equals(endpoint)) return "carinal";
    return null;
  }

  public static String getPassword(String endpoint) {
    if("pic#dtn".equals(endpoint)) return "Koolcat1";
    if("nersc#hopper".equals(endpoint)) return "Koolcat!2";
    if("dolson#scdemo_data".equals(endpoint)) return "";
    if("ranantha#cfngftp".equals(endpoint)) return "Koolcat!2";
    return null;
  }

  public static boolean getIsGlobusPath(String endpoint, String path) {
    if("pic#dtn".equals(endpoint) && path.startsWith("/pic")) return true;
    if("nersc#hopper".equals(endpoint)) return true;
    if("dolson#scdemo_data".equals(endpoint)) return true;
    if("ranantha#cfngftp".equals(endpoint)) return true;

    return false;
  }

  public void globusTransfer(
      String sourceGlobusEndpoint,
      String sourceHostname,
      String sourceDirectory,
      String targetGlobusEndpoint,
      String targetHostname,
      String destinationDirectory) {

    try{

      // connect to globus endpoints (source/dest)
      JSONTransferAPIClient client = connectToGlobusEndpoint(
          sourceGlobusEndpoint, 
          sourceHostname, 
          getUsername(sourceGlobusEndpoint),
          getPassword(sourceGlobusEndpoint));

      connectToGlobusEndpoint(
          targetGlobusEndpoint, 
          targetHostname,
          getUsername(targetGlobusEndpoint),
          getPassword(targetGlobusEndpoint));
      ...
      Result r = client.getResult("/transfer/submission_id");
      //	            Result r = globusOnlineClient.getResult("/transfer/submission_id");
      ...
      item.put("source_endpoint", sourceGlobusEndpoint.toLowerCase());
      item.put("source_path", sourceDirectory);
      item.put("destination_endpoint", targetGlobusEndpoint.toLowerCase());
      item.put("destination_path", destinationDirectory);
  ...
  private boolean globusDownload(String remoteParent, String remoteFile, File localFile, boolean isBinary, String hostEncoding, IProgressMonitor monitor) throws SystemMessageException {
    boolean success = false;
    String remotePath = concat(remoteParent, remoteFile);

    try {
      Object jobObj = FieldUtils.readField(monitor, "job", true);
      String className = jobObj.getClass().getName();
      if( "org.eclipse.rse.internal.ui.view.SystemDNDTransferRunnable".equals(className) ) {
        System.out.println("is a globus transfer");

        Object currentTargetObj = FieldUtils.readField(jobObj, "_currentTarget", true);
        if(currentTargetObj instanceof IRemoteFile) {

          IRemoteFile targetFileObj = (IRemoteFile)currentTargetObj;
          IHost targetHost = targetFileObj.getHost();
          //          String targetGlobalPath = getGlobusOnlineHostProperty(targetHost, "globalPath");
          //          String targetGlobusEndpoint = getGlobusOnlineHostProperty(targetHost, "globusEndpoint");
          String targetGlobusEndpoint = getEndpoint(targetHost.getName());
          String destinationFile = targetFileObj.getAbsolutePath() + "/" + remoteFile;

          //          if(destinationDir.startsWith(targetGlobalPath)) {
          if(getIsGlobusPath(targetGlobusEndpoint, destinationFile)) {

            String sourceFilePath = remotePath;
            String pathToHostDir = localFile.getAbsolutePath();
            // have to switch path separator char in case of windows client
            pathToHostDir = pathToHostDir.replace("\\", "/");
            pathToHostDir = pathToHostDir.replace(remotePath, "");
            String sourceHostname = pathToHostDir.substring(pathToHostDir.lastIndexOf('/')+1).toLowerCase();

            // get the host object from the sourceHostname
            IHost sourceHost = getHost(sourceHostname);
            //            String sourceGlobusEndpoint = getGlobusOnlineHostProperty(sourceHost,"globusEndpoint");
            String sourceGlobusEndpoint = getEndpoint(sourceHost.getName());

            if(getIsGlobusPath(sourceGlobusEndpoint, sourceFilePath)) {
              // connect to globus online
              transferer.authenticateToGlobusOnline("clansing", "Koolcat1");

              globusTransfer( sourceGlobusEndpoint, getMyProxyHost(sourceGlobusEndpoint), sourceFilePath,
                  targetGlobusEndpoint, getMyProxyHost(targetGlobusEndpoint), destinationFile);
              success = true;
            }
          }
        } else {
  ...
  public void download(String remoteParent, String remoteFile, File localFile, boolean isBinary, String hostEncoding, IProgressMonitor monitor) throws SystemMessageException
  {
    ...
    // TODO: globus download is flakey, so we are just using ssh for now!
//    if(globusDownload(remoteParent, remoteFile, localFile, isBinary, hostEncoding, monitor)) {
//      return;
//    }
