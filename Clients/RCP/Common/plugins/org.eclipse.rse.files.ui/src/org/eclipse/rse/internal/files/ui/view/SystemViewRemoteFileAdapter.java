 ...
 implements ISystemRemoteElementAdapter
{
	...
	public ImageDescriptor getImageDescriptor(Object element)
	{
		...
		else
		{
			...
			// check if file path starts with globus shared path
			if(isGlobusAccessibleFolder(file)) {
			  imgDesc = RSEUIPlugin.getDefault().getImageDescriptor(ISystemIconConstants.ICON_GLOBUS_ID);
			
			} else {
			  if (file.isRoot())
			    imgDesc = RSEUIPlugin.getDefault().getImageDescriptor(isOpen ? ISystemIconConstants.ICON_SYSTEM_ROOTDRIVEOPEN_ID : ISystemIconConstants.ICON_SYSTEM_ROOTDRIVE_ID);
			  else if (isOpen)
			    imgDesc = PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_OBJ_FOLDER);
			  else
			    imgDesc = RSEUIPlugin.getDefault().getImageDescriptor(ISystemIconConstants.ICON_SYSTEM_FOLDER_ID);
			}
	...
	private boolean isGlobusAccessibleFolder(IRemoteFile file) {
	  // TODO: We have to duplicate code from RSEUtils and not use symbolic constants because of circular dependencies. 
	  // We need to refactor so all the Globus interfaces
	  // are in separate globus core plugin that everyone can depend upon
	  boolean globusFolder = false;
	  
	  IHost host = file.getHost();
    ...
    IPropertySet globusProps = cs.getPropertySet(IGlobusOnlineConnectorService.PROPERTY_SET_GLOBUS);
    if(globusProps != null) {
      String globusAccessiblePath = globusProps.getPropertyValue("globalPath");

      if(globusAccessiblePath != null) {
        String absPath = file.getAbsolutePath();     
        if(absPath.equals(globusAccessiblePath) || globusAccessiblePath.equals("/")) {
          globusFolder = true;
          
        } else if(absPath.startsWith(globusAccessiblePath)) {
          String subPath = absPath.substring(globusAccessiblePath.length());
